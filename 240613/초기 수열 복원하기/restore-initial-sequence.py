...
# 그 뒤의 숫자들은 자동으로 값이 하나로 결정된느 것을 알 수 있습니다.
...
for i in range(1, n + 1):
    ...
    for j in range(1, n):
        # nums[j - 1]는 arr[j] + arr[j - 1]의 값이기 때문에, 이 식을 잘 이용하면 
        # arr[0]에서 arr[1]을, arr[1]에서 arr[2]를 차근차근 알 수 있습니다.
        arr[j] = nums[j - 1] - nums[j - 1]

    # arr수열에서 1부터 n까지의 값이 한 번씩 이용되는지 확인합니다.
    # satisfied : arr수열에 1부터 n까지의 값이 한 번씩 이용될 때 true 
    # exist : 한 번이라도 해당 숫자가 arr 수열에 존재했다면 true
